  ...
  <>
      ...
      if (index === 2) {
       ...
       return (
        <div key="fab" className="relative">
         {/* Ping ring */}
         <span className="absolute inset-0 rounded-full border-2 animate-ping-ring" style={{ borderColor: 'var(--theme-primary)' }} />
         <button
          ...
          style={{ background: 'linear-gradient(135deg, var(--theme-gradient-start), var(--theme-gradient-end))', boxShadow: '0 4px 20px var(--theme-primary-glow)' }}
         >
          <motion.div
         ...
         <AnimatePresence>
          {fabOpen && (
           ...
           >
            ...
            >
             <ClipboardList size={16} style={{ color: 'var(--theme-primary)' }} /> New Ticket
            </Link>
            <Link
             to="/laptops"
             onClick={() => setFabOpen(false)}
             className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-text-primary hover:bg-bg-surface transition-colors"
            >
             <Laptop size={16} style={{ color: 'var(--theme-secondary)' }} /> Add Laptop
            </Link>
            <Link
             to="/kb"
             onClick={() => setFabOpen(false)}
      ...
      return (
       <Link
        ...
        className={cn(
         ...
         active ? 'theme-active' : 'text-text-muted hover:text-text-secondary',
        )}
       >
        <item.icon
         size={22}
         style={active ? { filter: 'drop-shadow(0 0 6px var(--theme-primary-glow))' } : undefined}
        />
   ...
   <aside className="hidden md:flex fixed left-0 top-0 bottom-0 w-[72px] z-50 flex-col items-center py-4 bg-[#12121a]/90 backdrop-blur-xl border-r border-[rgba(255,255,255,0.06)]">
    ...
    <Link to="/" className="mb-6">
     <div className="w-10 h-10 rounded-xl flex items-center justify-center font-display font-bold text-lg" style={{ background: 'linear-gradient(135deg, var(--theme-gradient-start), var(--theme-gradient-end))', color: '#0a0a0f' }}>
      P
     </div>
    </Link>
     ...
     {sidebarItems.map((item) => {
      ...
      return (
       <Link
        ...
        style={active ? { color: 'var(--theme-primary)', background: 'var(--theme-primary-dim)' } : undefined}
        title={item.label}
       >
        <item.icon size={22} style={active ? { filter: 'drop-shadow(0 0 6px var(--theme-primary-glow))' } : undefined} />
       </Link>
      );
     })}
    </div>
   ...
   <span className="absolute inset-0 rounded-full border-2 animate-ping-ring" />
     <button
      onClick={() => setFabOpen(!fabOpen)}
      className={cn(
      ...
      {fabOpen && (
       ...
       >
        <Link to="/tickets" onClick={() => setFabOpen(false)} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-text-primary hover:bg-bg-surface">
         <ClipboardList size={14} style={{ color: 'var(--theme-primary)' }} /> New Ticket
        </Link>
        <Link to="/laptops" onClick={() => setFabOpen(false)} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-text-primary hover:bg-bg-surface">
         <Laptop size={14} style={{ color: 'var(--theme-secondary)' }} /> Add Laptop
        </Link>
