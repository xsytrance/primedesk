...
router.get('/', (req, res) => {
  ...
  const p1p2 = db.prepare("SELECT COUNT(*) c FROM tickets WHERE status IN ('Open','In Progress','Pending') AND priority IN ('P1','P2')").get().c;
  ...
  res.json({ myOpen, allOpen, p1p2, openedToday, closedToday, recent, msp, leaderboard });
});
